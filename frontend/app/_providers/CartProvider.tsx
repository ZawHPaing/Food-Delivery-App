...
// Cart Actions
type CartAction =
  | { type: "ADD_ITEM"; payload: { menuItem: MenuItem; restaurant: Pick<Restaurant, "id" | "name" | "image" | "latitude" | "longitude">; quantity?: number; specialInstructions?: string } }
...
interface CartContextValue extends CartState {
  // Actions
  addItem: (menuItem: MenuItem, restaurant: Pick<Restaurant, "id" | "name" | "image" | "latitude" | "longitude">, quantity?: number, specialInstructions?: string) => void;
...
const TAX_RATE = 0.08;
const BASE_DELIVERY_FEE = 1500; // Base fee in Myanmar (likely pyas or just cents-equivalent)
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // For dynamic delivery fee (accessing rider location if available)
  // We can't use DeliveryProvider directly here if we want to avoid circularity, 
  // but Providers.tsx shows CartProvider is nested INSIDE DeliveryProvider.
  // However, useDeliveryContext is only for riders. 
  // For checkout, we'll try to get the "nearest rider" location or use a default.
  const riderLat = 16.8661; // Center of Yangon fallback
  const riderLng = 96.1951;
  ...
  const computedValues = useMemo(() => {
    ...
    if (state.items.length > 0 && state.restaurantLat && state.restaurantLng) {
      // 1. Get Customer Location (from localStorage if set by page.tsx handleSaveLocation, or fallback)
      let customerLat = 16.8409;
      let customerLng = 96.1735;
      
      if (typeof window !== "undefined") {
        const storedLoc = localStorage.getItem("foodie.customer.location");
        if (storedLoc) {
          try {
            const parsed = JSON.parse(storedLoc);
            if (parsed.latitude && parsed.longitude) {
              customerLat = parsed.latitude;
              customerLng = parsed.longitude;
            }
          } catch (e) {}
        }
      }
      ...
      // D1: Rider to Restaurant
      const d1 = getDistanceKm(riderLat, riderLng, state.restaurantLat, state.restaurantLng);
      // D2: Restaurant to Customer
      const d2 = getDistanceKm(state.restaurantLat, state.restaurantLng, customerLat, customerLng);
      
      const totalDistance = d1 + d2;
      deliveryFeeCents = BASE_DELIVERY_FEE + Math.ceil(totalDistance) * RATE_PER_KM;
    }
  ...
  }, [state.items, state.restaurantLat, state.restaurantLng, riderLat, riderLng]);
