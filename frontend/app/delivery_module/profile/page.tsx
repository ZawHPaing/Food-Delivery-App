...
export default function ProfilePage() {
  ...
  // Fetch latest stats on mount to ensure deliveries and fees are up to date
  const hasFetched = useRef(false);
  useEffect(() => {
    if (isLoggedIn && user?.email && !hasFetched.current) {
      hasFetched.current = true;
      const token = localStorage.getItem('access_token');
      if (token) {
        fetch(`http://localhost:8000/delivery/profile?email=${encodeURIComponent(user.email)}`, {
          ...
          .then(data => {
            if (data) {
              updateProfile({
                ...
                cash_collected_cents: data.cash_collected_cents,
                rider: data.rider
              });
            }
          })
  ...
  const stats = [
    { icon: Package, label: 'Delivery count', value: String(deliveriesCount) },
